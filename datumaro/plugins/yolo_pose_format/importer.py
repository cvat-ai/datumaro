...
from datumaro.plugins.yolo_pose_format.extractor import YoloPoseExtractor
from datumaro.util.os_util import extract_subset_name_from_parent

from .format import YoloPosePath


class YoloPoseImporter(Importer):
    ...
    def _check_ann_file_impl(cls, fp: TextIOWrapper) -> bool:
        ...
        for line in fp:
            req_fields = line.rstrip("\n").split(" ")
            req_len = len(req_fields)
            break

        for line in fp:
            fields = line.rstrip("\n").split(" ")
            if len(fields) != req_len:
                raise DatasetImportError(
                    f"Yolo pose format txt file should have equal number of fields for each line, "

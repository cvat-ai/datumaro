...
from datumaro.components.annotation import Annotation, AnnotationType, LabelCategories, Polygon
...
class YoloSegmentationExtractor(SourceExtractor):
    class Subset(Extractor):
        def __init__(self, name: str, parent: YoloSegmentationExtractor):
            super().__init__()
            self._name = name
            self._parent = parent
    ...
    def _parse_annotations(
        ...
        for idx, line in enumerate(lines):
            try:
                ...
                if len(parts) % 2 != 1:
                    raise InvalidAnnotationError(
                        f"Polygon has invalid value count {len(parts) - 1}, "
                ...
                label_id = self._parse_field(parts[0], int, "pose label id")
                ...
                for i in range(1, len(parts), 2):
                    x = self._parse_field(parts[i], float, f"polygon point {i//2} x")
                    y = self._parse_field(parts[i + 1], float, f"polygon point {i//2} y")
                    polygon_points.append(x * image_width)
                    polygon_points.append(y * image_height)

                annotations.append(
                    Polygon(polygon_points, label=label_id, id=idx, group=idx)
                ) 
            except Exception as e:

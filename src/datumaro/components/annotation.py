...
class ExtractedMask(Mask):
    ...
    The advantage of this class is that we can create multiple binary mask
    but they share a single index mask source.

    Attributes:
        index_mask: Integer 2D Numpy array. Its pixel can indicate a label id (class)
            or an instance id.
...
class Shape(Annotation):
    ...
    def get_points(self) -> Optional[List[Tuple[float, float]]]:
        ...
        Returns:
            Optional[List[Tuple[float, float]]]: List of points as (x, y) tuples,
...
class PolyLine(Shape):
    ...
    This class represents a polyline shape, which is a series of connected line segments.
...
class Cuboid3d(Annotation):
    ...
    This class represents a 3D cuboid annotation with position, rotation, and scale.

    Attributes:
        _type (AnnotationType): The type of annotation, set to `AnnotationType.cuboid_3d`.
        _points (List[float]): List of float values representing the position,
    ...
    def __init__(self, position, rotation=None, scale=None, **kwargs):
        ...
        Args:
            ...
            rotation (List[float], optional): List of 3 float values
                representing the rotation [rx, ry, rz].
            scale (List[float], optional): List of 3 float values
                representing the scale [sx, sy, sz].
        """
...
class Polygon(Shape):
    ...
    Methods:
        ...
        _get_shoelace_area: Helper method to calculate the area of the polygon
...
class RotatedBbox(Shape):
    ...
    def __init__(self, cx, cy, w, h, r, *args, **kwargs):
        """
        Initialize the RotatedBbox with its center (cx, cy), width (w), height (h),
...
class Cuboid2D(Annotation):
    """
    Cuboid2D annotation class. This class represents a 3D bounding box
    defined by its point coordinates in the following way:
    ...
    def _get_denorm(Tr_velo_to_cam_homo):
        ...
        Args:
            Tr_velo_to_cam_homo (np.ndarray):
                Homogeneous (4x4) LiDAR-to-camera transformation matrix
        Returns:
            np.ndarray: vector"""
    ...
    def to_3d(self, P_inv: np.ndarray) -> tuple[np.ndarray, np.ndarray, float]:
        """Reconstructs 3D object Velodyne coordinates
...
class Points(Shape):
    ...
    def _visibility_validator(self, attribute, visibility):
        ...
        Raises:
            AssertionError: If the length of the visibility list
                does not match half the length of the points list.
        """
...
class Annotations(List[Annotation]):
    ...
    def get_semantic_seg_mask(
        ...
        Raises:
            ValueError: If there are no mask annotations or
                if there is an inconsistency in mask sizes.
        """
        ...
        for class_mask in class_masks:
            if class_mask.shape != semantic_seg_mask.shape:
                msg = (
                    f"There is inconsistency in mask size: "
                    f"{class_mask.shape}!={semantic_seg_mask.shape}."
                )

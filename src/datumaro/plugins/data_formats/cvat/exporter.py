        ...
        for l_id, l in enumerate(label_cat.items):
            ...
            if points_cat.items.get(l_id):
                ...
                for label_from, label_to in points_cat.items[l_id].joints:
                    label["svg"] += (
                        f'<line x1="0" y1="0" x2="0" y2="0"data-type="edge" data-node-from="{label_from + 1}" '
                        f'stroke-width="0.5" data-node-to="{label_to + 1}"></line>'
                    )
                for i, sublabel in enumerate(points_cat.items[l_id].labels):
                    label["svg"] += (
                        f'<circle r="1.5" stroke="black" fill="#b3b3b3" cx="0" cy="0" stroke-width="0.1" '
                        f'data-type="element node" data-element-id="{i + 1}" data-node-id="{i + 1}" '
                        f'data-label-name="{sublabel}"></circle>'
                    )

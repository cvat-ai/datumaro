...
class ImagenetExporter(Exporter):
    ...
    def _apply_impl(self):
        ...
        n_of_subsets = len(self._extractor.subsets())
        if 1 < n_of_subsets and not self.USE_SUBSET_DIRS:
            log.warning(
                f"There are more than one subset in the dataset ({n_of_subsets}). "
                "However, ImageNet format exports all dataset items into the same directory. "
                "Therefore, subset information will be lost. "
                "To prevent it, please use ImagenetWithSubsetDirsExporter. For example, "
                'dataset.export("<path/to/output>", format="imagenet_with_subset_dirs").'
            )

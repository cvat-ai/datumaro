...
class TelemetryTest(TestCase):
    ...
    def test_send_command_success_info(self, mock_send_event):
        cli_args = types.SimpleNamespace(
            loglevel="20", target="project", project_dir="/a/b/c", command=info_command
        )

        send_command_success_info(
            self.telemetry, cli_args, sensitive_args=("project_dir", "target")
        )
        ...
        mock_send_event.assert_any_call(
            ...
            "info_result",
            json.dumps(
                {"status": "success", "loglevel": "20", "target": "True", "project_dir": "True"}
            ),
        )
    ...
    def test_send_command_failure_info(self, mock_send_event):
        ...
        mock_send_event.assert_any_call(
            ...
            "info_result",
            json.dumps(
                {"status": "failure", "loglevel": "20", "target": "True", "project_dir": "True"}
            ),
        )
    ...
    def test_send_command_exception_info(self, mock_send_event):
        ...
        mock_send_event.assert_any_call(
            ...
            "info_result",
            json.dumps(
                {"status": "exception", "loglevel": "20", "target": "True", "project_dir": "True"}
            ),
        )
